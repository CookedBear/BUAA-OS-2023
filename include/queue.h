 ...
 * structure con‐ tains a single pointer to the first element on the list.  The
 * elements are doubly linked so that an arbitrary element can be removed
 * without traversing the list.  New elements can be added to the list after an
 * existing element or at the head of the list. A LIST_HEAD structure is
 * declared as follows:
 *
 *       LIST_HEAD(HEADNAME, TYPE) head;
 *
 * where HEADNAME is the name of the structure to be defined, and TYPE is the
 * type of the elements to be linked into the list.
 */
