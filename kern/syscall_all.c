...
void get_childs(u_int envid, u_int childs[]);
...
int sys_ipc_broadcast(u_int val, void *srcva, u_int perm) {
  ...
  // get_childs(curenv->env_id, childs);
...
void get_childs(u_int envid, u_int childs[]) {
  ...
  for (int i = 0; e->child_envid[i] != 0; i++) {
    get_childs(e->child_envid[i], childs);
  }
