...
int sys_init(const char *name, int init_value, int checkperm) {
  ...
  sems[idList].id = idList;
  sems[idList].value = init_value;
  sems[idList].checkperm = (0 == checkperm) ? 0 : sys_getenvid();
  memcpy(sems[idList].name, name, sizeof(name));
  idList++;
  return (idList - 1);
}

int sys_wait(int sem_id) {
  if (sem_id > 10 || sem_id < 0) {
    return -E_NO_SEM;
  }
  if (sems[sem_id].id != sem_id) {
    return -E_NO_SEM;
  }
  if (sems[sem_id].checkperm != 0) {
    struct Env *e;
    int f = 0;
    envid2env(sys_getenvid(), &e, 0);
    while (e->env_id != 0) {
      if (e->env_id == sems[sem_id].checkperm) {
        f = 1;
        break;
      }
...
int sys_getid(const char *name) {
  ...
  for (i = 1; i <= 11; i++) {
    ...
    if (sems[i].checkperm != 0) {
      struct Env *e;
      int f = 0;
      envid2env(sys_getenvid(), &e, 0);
      while (e->env_id != 0) {
        if (e->env_id == sems[i].checkperm) {
          f = 1;
          break;
        }
        if (e->env_parent_id == 0) {
...
/* Overview:
 *   Allocate a physical page and map 'va' to it with 'perm' in the address
 * space of 'envid'. If 'va' is already mapped, that original page is sliently
 * unmapped. 'envid2env' should be used with 'checkperm' set, like in most
 * syscalls, to ensure the target is either the caller or its child.
 *
 * Post-Condition:
 *   Return 0 on success.
 *   Return -E_BAD_ENV: 'checkperm' of 'envid2env' fails for 'envid'.
 *   Return -E_INVAL:   'va' is illegal (should be checked using
 * 'is_illegal_va'). Return the original error: underlying calls fail (you can
 * use 'try' macro).
 *
 * Hint:
...
int sys_mem_alloc(u_int envid, u_int va, u_int perm) {
  ...
  if (0 != envid2env(envid, &env,
                     1)) { // test 4-2: !envid2env() or 0 != envid2env())
    return -E_BAD_ENV;
  }
...
/* Overview:
 *   Find the physical page mapped at 'srcva' in the address space of env
 * 'srcid', and map 'dstid''s 'dstva' to it with 'perm'.
 *
 * Post-Condition:
 *   Return 0 on success.
 *   Return -E_BAD_ENV: 'checkperm' of 'envid2env' fails for 'srcid' or 'dstid'.
 *   Return -E_INVAL: 'srcva' or 'dstva' is illegal, or 'srcva' is unmapped in
 * 'srcid'. Return the original error: underlying calls fail.
 *
 * Hint:
 *   You may want to use the following functions:
 ...
 *   - The new env's 'env_tf' is copied from the kernel stack, except for $v0
 * set to 0 to indicate the return value in child.
 ...
 * 'ENV_NOT_RUNNABLE'. Returns the original error if underlying calls fail.
 *
 * Hint:
 ...
 *  Returns 0 on success (except when the 'envid' is the current env, so no
 * value could be returned). Returns -E_INVAL if the environment cannot be
 * manipulated or 'tf' is invalid. Returns the original error if other
 ...
 * other envs. 'curenv' is blocked until a message is sent.
 *
 * Post-Condition:
 *   Return 0 on success.
 ...
 * message with 'sys_ipc_recv'. Return the original error when underlying calls
 * fail.
 */
int sys_ipc_try_send(u_int envid, u_int value, u_int srcva, u_int perm) {
  ...
  if (srcva != 0) {
    ...
    // srcva, e->env_id, e->env_ipc_dstva, perm);
    // try(sys_mem_map(curenv->env_id, srcva, e->env_id, e->env_ipc_dstva,
    // perm)); Test 4-5: do not use sys_mem_map() because it call function
    // envid2env with perm == 1; while two env communicating they may not have
    // parent relatioinship
 ...
 *physical address 'pa'. Remember to check the validity of 'va' and 'pa' (see
 *Hint below);
 *
 ...
 *to perform MMIO. Hint: You can use 'is_illegal_va_range' to validate 'va'.
 ...
 *   The possible arguments are stored at $a1, $a2, $a3, [$sp + 16 bytes], [$sp
