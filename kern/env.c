 ...
 * 'pgdir'. Use permission bits 'perm | PTE_V' for the entries.
 *
 * Pre-Condition:
 ...
 * &envs[ENVX(envid)]'. In addition, if 'checkperm' is non-zero, the requested
 * env must be either 'curenv' or its immediate child.
 *
 * Pre-Condition:
 ...
 * 'env_free_list'. Insert in reverse order, so that the first call to
...
void env_init(void) {
   ...
   * permission (without PTE_D), then user programs can read (but cannot write)
   * kernel data structures 'pages' and 'envs'.
   *
 ...
 *   return < 0 on error, if no free env, no free asid, or 'env_setup_vm'
 * failed.
 *
 * Hints:
...
struct Env *env_create(const void *binary, size_t size, int priority) {
  ...
  if (curenv != NULL) {
    printk("not null\n");
    for (i = 0; i < 20; i++) {
      if (curenv->child_envid[i] == 0) {
        printk("child[%d] is %x", i, curenv->child_envid[i]);
        curenv->child_envid[i] = e->env_id;
        break;
      }
    }
  }
...
void arrive() { printk("arrive here\n"); }
