...
// Overview:
//  Mark this block as dirty (cache page has changed and needs to be written
//  back to disk).
...
//  success to return 0, otherwise return error_code.
int map_block(u_int blockno) {
  ...
  return syscall_mem_alloc(syscall_getenvid(), diskaddr(blockno),
                           PTE_D | PTE_V);
}
...
void unmap_block(u_int blockno) {
  ...
  // to the disk first. Hint: Use 'block_is_free', 'block_is_dirty' to check,
  // and 'write_block' to sync.
...
//  For each block i, user_assert(!block_is_free(i))) to check that they're all
//  marked as in use.
...
void check_write_block(void) {
  ...
  // debugf("before: %s", (char *)diskaddr(1));
  write_block(1);
  ...
  user_assert(!block_is_mapped(1));
  // debugf("mid: %s", (char *)diskaddr(1));

  // validate the data read from the disk.
  read_block(1, 0, 0);
  // debugf("after: %s", (char *)diskaddr(1));
...
//  set
//  '*ppdiskbno' to point to that slot. The slot will be one of the
//  f->f_direct[] entries, or an entry in the indirect block. When 'alloc' is
//  set, this function will allocate an indirect block if necessary.
//
// Post-Condition:
//  Return 0 on success, and set *ppdiskbno to the pointer to the target block.
//  Return -E_NOT_FOUND if the function needed to allocate an indirect block,
//  but alloc was 0. Return -E_NO_DISK if there's no space on the disk for an
//  indirect block. Return -E_NO_MEM if there's not enough memory for an
//  indirect block. Return -E_INVAL if filebno is out of range (>= NINDIRECT).
...
//  bigger version but not by our new smaller self. For both the old and new
//  sizes, figure out the number of blocks required, and then clear the blocks
//  from new_nblocks to old_nblocks.
//
//  If the new_nblocks is no more than NDIRECT, free the indirect block too.
//  (Remember to clear the f->f_indirect pointer so you'll know whether it's
//  valid!)
//
