...
int spawn(char *prog, char **argv) {
	...
	ELF_FOREACH_PHDR_OFF (ph_off, ehdr) {
		...
		if ((r = readn(fd, elfbuf, ehdr->e_phentsize)) < 0) { goto err1; }
		Elf32_Phdr *ph = (Elf32_Phdr *)elfbuf;
		if (ph->p_type == PT_LOAD) {
			void *bin;
			// Read and map the ELF data in the file at 'ph_off' into our memory
			// using 'read_map()'.
			// 'goto err1' if that fails.
			...
			if ((r = elf_load_seg(ph, bin, spawn_mapper, &child)) < 0) { goto err1; }
		}
	}
	close(fd);
