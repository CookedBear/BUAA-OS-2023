...
// Overview:
//  Read 'n' bytes from 'fd' at the current seek position into 'buf'. Since
//  files are memory-mapped, this amounts to a memcpy() surrounded by a little
//  red tape to handle the file size and seek pointer.
static int file_read(struct Fd *fd, void *buf, u_int n, u_int offset) {
  u_int size;
  struct Filefd *f;
  f = (struct Filefd *)fd;
